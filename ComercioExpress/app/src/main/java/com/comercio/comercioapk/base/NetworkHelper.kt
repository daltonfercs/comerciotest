...
sealed class Resultado<out T> {
    ...
    object NetworkError : Resultado<Nothing>()
}
...
class ResultAdapter(
    ...
    override fun adapt(call: Call<Type>): Call<Resultado<Type>> = ResultCall(call)
}
        ...
        proxy.enqueue(object : Callback<T> {
            override fun onResponse(call: Call<T>, response: Response<T>) {
                ...
                val result = if (response.isSuccessful) {
                    ...
                    Resultado.Success<T>(body)
                } else {
                    val responseError =
                        GsonBuilder().create().fromJson(
                            response.errorBody()!!.string(), ErrorResponse::class.java
                        )
                    Resultado.Failure(code,responseError)
                }
